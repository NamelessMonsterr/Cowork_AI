...
ALLOWED_PATHS = [os.getenv("APPDATA"), os.getenv("TEMP")]
...
def apply_patches():
    ...
    if "subprocess" in sys.modules:
        sys.modules["subprocess"].Popen = lambda *args, **kwargs: (_ for _ in ()).throw(
            PermissionError("subprocess blocked")
        )
        sys.modules["subprocess"].run = lambda *args, **kwargs: (_ for _ in ()).throw(
            PermissionError("subprocess blocked")
        )
