...
class ActionBudget:
        ...
        with self._lock:
            if self._state.is_paused:
                raise BudgetExceededError(
                    f"Execution paused: {self._state.pause_reason}",
                    "paused",
                    0, 0
                )
            ...
            if self._state.actions_executed >= self._config.max_actions_per_task:
                self._trigger_exceeded("actions", self._state.actions_executed, self._config.max_actions_per_task)
            
            # Check retries
            if self._state.retries_attempted >= self._config.max_retries_per_task:
                self._trigger_exceeded("retries", self._state.retries_attempted, self._config.max_retries_per_task)
    ...
    def get_remaining(self) -> dict:
        ...
        with self._lock:
            ...
            return {
                ...
                "failures_until_pause": max(0, self._config.max_consecutive_failures - self._state.consecutive_failures),
            }
    ...
    def _trigger_exceeded(self, budget_type: str, current: int, limit: int) -> None:
        ...
        if self._on_exceeded:
            ...
            threading.Thread(
                target=self._on_exceeded,
                args=(self._state.pause_reason,),
                daemon=True
            ).start()
