                ...
                for arg_val in step.args.values():
                    ...
                    for pattern in self.dangerous_patterns:
                        if re.search(pattern, val_lower):
                            raise ValueError(f"⚠️ SAFETY BLOCK: Destructive command detected in step {step.id}: '{arg_val}'. Automatic execution denied.")
                ...
                for arg_val in step.args.values():
                    ...
                    if ("*" in arg_val or "?" in arg_val):
                        ...
                        if "del " in arg_val.lower() or "rm " in arg_val.lower():
                             raise ValueError(f"⚠️ SAFETY BLOCK: Wildcard deletion detected in step {step.id}: '{arg_val}'. Too risky for beta.")
