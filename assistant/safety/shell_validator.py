...
class RestrictedShellValidator:
    ...
    BLOCKED_PS_FLAGS = {
        "-enc",
        "-encodedcommand",
        "-e",  # Encoded command execution
        "-executionpolicy",
        "-ep",
        "-ex",  # Execution policy bypass
        "bypass",
        "unrestricted",
        "remotesigned",  # Policy levels
    }
    ...
    def _validate_powershell(self, first_token: str, command: str) -> None:
        ...
        for flag in self.BLOCKED_PS_FLAGS:
            if flag in command_lower:
                raise SecurityError(f"Blocked PowerShell flag detected: '{flag}'")

        # Check for blocked cmdlets
        for cmdlet in self.BLOCKED_PS_CMDLETS:
            if cmdlet in command_lower:
                raise SecurityError(f"Blocked PowerShell cmdlet detected: '{cmdlet}'")
