...
# CRITICAL SECURITY FIX: Import size constraints to prevent DoS
...
class ActionStep(BaseModel):
    ...
    CRITICAL SECURITY FIX: Added size limits to prevent DoS
    """

    id: constr(max_length=128) = Field(..., description="Unique step identifier")  # type: ignore
    tool: constr(max_length=128) = Field(..., description="Name of the tool/action to execute")  # type: ignore
    ...
    # UI display
    description: constr(max_length=2048) | None = Field(default=None, description="Human-readable description")  # type: ignore
...
class ExecutionPlan(BaseModel):
    ...
    CRITICAL SECURITY FIX: Added size limits to prevent massive payloads
    """

    id: constr(max_length=128) = Field(..., description="Unique plan identifier")  # type: ignore
    task: constr(max_length=2048) = Field(..., description="What this plan accomplishes")  # type: ignore
    steps: conlist(ActionStep, max_length=100) = Field(..., description="Ordered list of steps")  # type: ignore
    ...
    # For display
    summary: constr(max_length=4096) | None = None  # type: ignore
