...
async def benchmark_validation():
    ...
    plan = ExecutionPlan(
        ...
        steps=[
            ActionStep(step_id="1", tool="click", params={"x": 100, "y": 200})
        ]
    )
...
def run_benchmarks():
    ...
    print("=" * 60)
    total_score = 0
    if startup_ms < 1000: total_score += 50
    elif startup_ms < 2000: total_score += 40
    else: total_score += 30
    
    if validation_ms < 10: total_score += 50
    elif validation_ms < 20: total_score += 40
    else: total_score += 30
