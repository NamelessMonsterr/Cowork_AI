...
from assistant.computer import Computer, contrib, default
...
def get_required_attrs():
    ...
    return [
        ...
        if not name.startswith("__") and (inspect.isfunction(member) or isinstance(member, property))
    ]
...
def test_computer_implements_interface(computer_class):
    for func in get_required_attrs():
        assert hasattr(computer_class, func), f"{computer_class.__name__} is missing required attribute '{func}'"
